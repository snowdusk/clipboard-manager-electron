...
const openJsonFormatter = require('./jsonFormatter/main')
const openSqlFormatter = require('./sqlFormatter/main')
const openJsonDiff = require('./jsondiff/main')
const openTimestamp = require('./timestamp/main')
const openUrl = require('./url/main')
const openString = require('./string/main')
...
function submenuHistoryLimitTemplate () {
  const copiedQty = getCopiedQty()
  const limits = [1, 10, 50, 100, 500, 1000, 10000]
  ...
  return limits.map(limit => {
    ...
    return {
      ...
      click: () => {
        ...
        if (copiedLength > newCopiedQty) { // 减小保存数量
...
function getCopiedFromDbWithLabel () {
  ...
  copied = copied.map(currentText => {
    return {
      text: currentText,
      label: truncateCurrentText(currentText, 50, false),
    }
  })
  return { copied, length }
}
